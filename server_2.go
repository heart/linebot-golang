...
func main() {
	...
	// serve /static/** files
	staticFileServer := http.FileServer(http.Dir("assets"))
	http.HandleFunc("/assets/", http.StripPrefix("/assets/", staticFileServer).ServeHTTP)
	...
	http.HandleFunc("/webhook", app.Callback)
